...
{
    public sealed record GeneralSettingsProductInfo(string ProductName, string SerialNumber, string ProgramVersion);
    public sealed record GeneralSettingsPageData(GeneralSettingsProductInfo Product, ChikoTpSession.GeneralSettingsSnapshot Settings);
    // ★DI（OSごとに実体が入る想定）
...
=> new LineSeries<ObservablePoint>
{
    ...
    private async Task OpenOperationHistoryAsync()
    {
        try
        {
            ...
            await Shell.Current.Navigation.PushAsync(new OperationHistoryPage(this));
        }
        catch (Exception ex)
        {
    ...
    public async Task<GeneralSettingsPageData> LoadGeneralSettingsPageDataAsync(CancellationToken ct)
    {
        Log("[GENERAL] read S7");
        Log("[GENERAL] read S5");

        return await ExecuteGeneralSettingsIoAsync(async (session, token) =>
        {
            var product = await session.ReadProductInfoAsync(token);
            var settings = await session.ReadGeneralSettingsAsync(token);

            return new GeneralSettingsPageData(
                new GeneralSettingsProductInfo(product.ProductName, product.SerialNumber, product.ProgramVersion),
                settings);
        }, ct);
    }

    public Task<ChikoTpSession.GeneralSettingsSnapshot> LoadGeneralSettingsAsync(CancellationToken ct)
    {
        Log("[GENERAL] read S5");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.ReadGeneralSettingsAsync(token), ct);
    }

    public Task SaveVolumeDownRateAsync(int value, CancellationToken ct)
    {
        Log($"[GENERAL] send volume down rate: {value}");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.WriteVolumeDownRateAsync(value, token), ct);
    }

    public Task SaveRemoteOutputSignalAsync(int value, CancellationToken ct)
    {
        Log($"[GENERAL] send remote output: {value}");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.WriteRemoteOutputSignalAsync(value, token), ct);
    }

    public Task SaveShakingIntervalAsync(int value, CancellationToken ct)
    {
        Log($"[GENERAL] send shaking interval: {value}");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.WriteShakingIntervalAsync(value, token), ct);
    }

    public Task SaveShakingOperatingTimeAsync(int value, CancellationToken ct)
    {
        Log($"[GENERAL] send shaking operating time: {value}");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.WriteShakingOperatingTimeAsync(value, token), ct);
    }

    public Task SavePulseIntervalAsync(int value, CancellationToken ct)
    {
        Log($"[GENERAL] send pulse interval: {value}");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.WritePulseIntervalAsync(value, token), ct);
    }

    public Task SaveAutoPulseAsync(bool value, CancellationToken ct)
    {
        Log($"[GENERAL] send auto pulse: {value}");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.WriteAutoPulseAsync(value, token), ct);
    }

    public Task ClearInitialAirVolumeFromSettingsAsync(CancellationToken ct)
    {
        Log("[GENERAL] send clear initial air volume");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.ClearInitialAirVolumeAsync(token), ct);
    }

    public Task TriggerManualPulseFromSettingsAsync(CancellationToken ct)
    {
        Log("[GENERAL] send manual pulse");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.TriggerManualPulseAsync(token), ct);
    }

    public Task ResetSettingValuesFromSettingsAsync(CancellationToken ct)
    {
        Log("[GENERAL] send reset setting value");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.ResetSettingValueAsync(token), ct);
    }

    public Task<IReadOnlyList<ChikoTpSession.LogHistoryItem>> ReadOperationHistoryAsync(CancellationToken ct, IProgress<int>? progress = null)
    {
        Log("[LOG] read operation history S2");
        return ExecuteGeneralSettingsIoAsync((session, token) => session.ReadLogHistoryAsync(token, progress), ct);
    }

    private async Task ExecuteGeneralSettingsIoAsync(Func<ChikoTpSession, CancellationToken, Task> action, CancellationToken ct)
    {
        await ExecuteGeneralSettingsIoAsync(async (session, token) =>
        {
    ...
    private async Task<T> ExecuteGeneralSettingsIoAsync<T>(Func<ChikoTpSession, CancellationToken, Task<T>> action, CancellationToken ct)
    {
        await _settingsIoLock.WaitAsync(ct);

        var resumePolling = false;
        try
        {
            if (!IsConnected || _tp is null)
                throw new InvalidOperationException("Device is not connected.");

            var session = _tp;

            resumePolling = _s4Timer != null && _monitorCts is { IsCancellationRequested: false };
            if (resumePolling)
                StopS4Polling();
        ...
        {
            if (resumePolling && IsConnected && _tp is not null && _monitorCts is { IsCancellationRequested: false })
            {
                StartS4Polling(_monitorCts.Token);
            }
